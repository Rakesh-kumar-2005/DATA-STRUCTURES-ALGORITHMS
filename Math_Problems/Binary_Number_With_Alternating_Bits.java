...
Description:
  Following program determines whether the binary representation of a given integer contains alternating bits (0s and 1s)...

Problem Statement:
  -> Given a positive integer n, check whether its binary representation has alternating bits...
  -> Adjacent bits must not be the same...
  -> Valid patterns include 101, 1010, 10101...
  -> Invalid patterns include 100, 111, 1001...
  -> Return true if bits alternate, otherwise return false...

Core Idea:
  -> Convert the integer into its binary string form...
  -> Compare each bit with its immediate neighbor...
  -> If any two adjacent bits are equal, the pattern is not alternating...

Approach:
  > String-Based Pattern Checking:
     i. Convert integer to binary using Integer.toBinaryString(n)...
     ii. Traverse the binary string from index 1 onward...
     iii. Compare current bit with the previous bit...
     iv. If both are equal, return false immediately...
     v. If traversal completes, return true...

> Binary Conversion Logic:
  -> Integer.toBinaryString(n) converts integer to minimal binary representation...
...
> Alternating Pattern Rule:
  -> Valid if: bit[i] ≠ bit[i−1] for all valid indices...
  -> Invalid if: any adjacent pair matches...

> Algorithm Steps:
  -> Convert integer to binary string...
  -> Loop from i = 1 to length − 1:
       * Compare number.charAt(i) with number.charAt(i−1)...
       * If equal → return false...
  -> Return true after successful comparisons...
...
> Step Execution:
  ...
  -> Compare adjacent characters sequentially...
  -> Exit early if violation found...
  -> Otherwise confirm alternating pattern...

> Edge Cases:
  -> Single-bit numbers (e.g., 1) → true (no adjacent bits)...
  -> Two-bit numbers (10) → true...
  -> Numbers with consecutive 0s or 1s → false...
  -> Large numbers handled correctly due to string traversal...

> Alternative Optimal Approach:
  -> Use bit manipulation:
       n ^ (n >> 1) should produce all 1s for alternating bits...
  -> Faster and constant space, but less intuitive...

> Implementation Notes:
  -> charAt() compares character values directly...
  -> '0' and '1' represent binary digits...
  -> Early return improves efficiency when pattern fails...

> Time and Space Complexity:
  -> Time Complexity: O(log n), proportional to number of binary digits...
  -> Space Complexity: O(log n), for binary string storage...

> Applications:
  -> Digital signal pattern validation...
  -> Error detection in binary communication...
  -> Bit pattern verification in embedded systems...
  -> Coding interview and bit manipulation exercises...
