...
Description:
  This program counts how many **monobit integers (powers of two)** exist
  in the range 0 to n (inclusive)...

Definition:
  -> Monobit integer = number with exactly ONE bit set to 1...
  -> Also called power of 2...
  -> Form: 2^k...
...
Goal:
  -> Count all numbers ≤ n that satisfy:
       (i & (i - 1)) == 0...

Key Bit Trick:
  -> For power of 2:
       i & (i-1) removes the only set bit...
       result becomes 0...
  -> Otherwise:
       result ≠ 0...
...
Method 1: countMonobitBF()  → Brute Force...
  -> Iterate from 1 to n...
  -> Check each number using (i & (i-1)) == 0...
...
Method 2: countMonobit()  → Bit-length Optimized...
  -> Uses Integer.numberOfLeadingZeros(n)...
  -> Finds how many bits required to represent n...
  -> Formula:
       bitsNeeded = 32 - leadingZeros...
       answer = bitsNeeded + 1...

  -> Logic:
       if highest bit index = k...
       powers are:
         2^0, 2^1, ..., 2^k...
       total = k + 2 (including 0)...
...
Method 3: countMonobitAlternative()  → Logarithmic...
  -> Uses:
       floor(log₂(n)) + 2...
  -> Finds highest power mathematically...
  -> Works similar to bit-count method...
  -> Time Complexity:  O(1)...
  -> Space Complexity: O(1)...

------------------------------------------------------------

Method 4: countMonobitIterative()  → Power Doubling...
  -> Start from powerOfTwo = 2...
  -> Keep doubling using bit shift:
       powerOfTwo <<= 1...
  -> Count until exceeding n...
...
General Rule:
  -> Each time n crosses a new power of 2...
...
------------------------------------------------------------
  ...
  -> n = 0  → only {0}...
  -> n = 1  → {0,1}...
  -> negative or zero handled safely...
...
------------------------------------------------------------
  ...
  Brute Force        → O(n)...
  Iterative Power    → O(log n)...
  Logarithmic        → O(1)...
  Bit-length method  → O(1) ⭐ Best...

------------------------------------------------------------
Key Insight:
------------------------------------------------------------

  Counting monobit numbers is same as:
    counting number of powers of two ≤ n...

  Instead of checking every number...
  simply find highest set bit...

------------------------------------------------------------
Final Verdict:
------------------------------------------------------------

  -> Use bit-length method for best performance...
  -> Constant time...
  -> Clean and efficient...
  -> Ideal for interviews and large inputs...
