...
Description:
  Following program computes the minimum number of steps required to reduce a binary number (given as a string) to 1 using specific operations...

Problem Statement:
  -> Given a binary string representing a positive integer...
  -> Reduce the number to "1" using the following rules:
       • If the number is even → divide by 2...
       • If the number is odd  → add 1...
  -> Return the minimum number of steps required...

Operation Rules in Binary:
  -> Even number (ends with 0):
       divide by 2 → remove last bit (right shift)...
  -> Odd number (ends with 1):
       add 1 → causes carry propagation...
       then divide by 2...
...
Core Insight:
  -> Instead of simulating operations, process bits from right to left...
  -> Track carry caused by add operations...
  -> Compute steps using bit patterns...

Binary Operation Cost:
  -> Even bit → 1 step (divide)...
  -> Odd bit  → 2 steps (add + divide)...
  -> Carry may propagate to next bit...

Approach:
  > Right-to-Left Processing with Carry:

     i. Initialize:
          steps = 0...
          carry = 0...
     ii. Traverse binary string from rightmost bit to index 1...
     iii. For each bit:
            digit = bit + carry...
     iv. If digit == 1 (odd):
            steps += 2...
     ...
     v. Else (digit == 0 or 2):
            steps += 1...
     vi. After loop, return steps + carry...

Why Skip Leftmost Bit?
  -> Loop runs while i > 0...
  -> Leftmost bit represents final "1"...
  -> Processing it would add unnecessary step...

Digit + Carry Interpretation:
  -> 0 + 0 = 0 → even → divide → +1 step...
  -> 1 + 0 = 1 → odd  → add & divide → +2 steps, carry=1...
  -> 0 + 1 = 1 → odd due to carry → +2 steps...
  -> 1 + 1 = 2 → even → divide → +1 step (carry remains)...

Algorithm Steps:
  -> Start from least significant bit...
  -> Track carry propagation...
  -> Count operations logically...
  -> Add final carry if present...

Example Walkthrough: "1101"
  -> carry=0, steps=0

  bit=1 → digit=1 → +2 steps, carry=1 → steps=2
  bit=0 → digit=1 → +2 steps → steps=4
  bit=1 → digit=2 → +1 step  → steps=5
  end → add carry → steps=6

  Output: 6...

Why Add Final Carry?
  -> Carry after processing means overflow beyond MSB...
  -> Requires one final division to remove it...

Patterns & Observations:
  -> Power of 2 (1000):
       only divisions needed...
       steps = number of trailing zeros...
  -> All ones (1111):
       first add creates power of 2...
       remaining operations are divisions...
  -> Alternating bits cause frequent carry propagation...

Character to Digit Conversion:
  -> str.charAt(i) - '0'
  -> converts '0' → 0 and '1' → 1...

Edge Cases:
  -> "1" → already reduced → 0 steps...
  -> Single trailing zeros → only divisions...
  -> Long carry chains handled efficiently...
  -> Works for very large binary strings...

Why This Method is Efficient:
  -> Avoids repeated binary addition simulation...
  -> Avoids large integer conversion limits...
  -> Processes each bit exactly once...
...
Advantages Over Simulation:
  -> No string mutation required...
  -> No big integer arithmetic...
  -> Handles arbitrarily large binary numbers...
  -> Optimal linear-time solution...

Applications:
  -> Binary arithmetic optimization...
  -> Bit-level algorithm design...
  -> Efficient handling of large numeric representations...
