...
Description:
  Following program sorts integers based on the number of 1-bits in their binary representation, and resolves ties using numeric value ordering...

Problem Statement:
  -> Given an array of integers...
  -> Sort elements by the count of set bits (1s) in their binary form...
  -> If two numbers have the same bit count, sort them by their numeric value...
  -> Return the sorted array...

Core Sorting Rule:
  -> Primary key  : number of set bits (ascending)...
  -> Secondary key: numeric value (ascending)...
...
Core Idea:
  -> Compute bit count for each number...
  -> Use a priority queue (min-heap) with a custom comparator...
  -> Comparator sorts first by bit count, then by numeric value...
  -> Extract elements to obtain sorted order...

Data Structure Used:
  -> Pair class stores:
       • num      → original number...
       • bitCount → number of 1-bits...

Why Store Pair?
  -> Avoid recomputing bit counts...
  -> Enables multi-key sorting efficiently...
...
Approach:
  > Priority Queue with Custom Comparator:
     ...
     i. Create min-heap storing Pair objects...
     ii. For each number:
           compute bit count...
           insert (number, bitCount) into heap...
     iii. Extract elements from heap in sorted order...
     iv. Store extracted numbers into result array...
...
Comparison Examples:
  -> Compare 2 (1 bit) vs 3 (2 bits):
       1 - 2 = -1 → 2 first...
  -> Compare 5 (2 bits) vs 3 (2 bits):
       5 - 3 = 2 → 3 first...
  -> Compare 7 (3 bits) vs 6 (2 bits):
       3 - 2 = 1 → 6 first...
...
Walkthrough Example:
  ...
  Step 1: Compute bit counts
    3→2 bits
    5→2 bits
    6→2 bits
    2→1 bit

  Step 2: Heap ordering
    (2,1) → smallest bit count
    (3,2), (5,2), (6,2)
...
Edge Cases:
  -> All numbers have same bit count → sorted numerically...
  -> Array contains duplicates → preserved correctly...
  -> Single element → unchanged...
  -> Zero included → bit count = 0...
  -> Large numbers handled efficiently...

Alternative Approach:
  -> Use Arrays.sort with custom comparator...
  -> More concise but conceptually similar...
  -> Priority queue useful for streaming or incremental sorting...

Implementation Notes:
  -> Priority queue ensures automatic ordering...
  -> Pair avoids repeated bit counting...
  -> Integer.bitCount is highly optimized...
  -> Heap extraction guarantees sorted sequence...

Time and Space Complexity:
  -> Time Complexity: O(n log n)...
       • inserting n elements into heap...
       • removing n elements from heap...
  -> Space Complexity: O(n)...
       • heap storage and result array...

Applications:
  -> Bit manipulation based ordering...
  -> Memory-efficient encoding comparisons...
