...
Description:
  This program counts the total number of negative numbers present in a 2D matrix that is 
      sorted in descending order both row-wise and column-wise...

Problem Statement:
  -> You are given an m × n matrix grid where each row and each column is sorted in non-increasing (descending) order...
  -> Your task is to count how many elements in the matrix are strictly negative...
  -> The solution must be efficient and avoid checking every element individually...

Key Insight:
  -> Because the matrix is sorted row-wise and column-wise, negatives form a “staircase” pattern...
  -> Starting from the bottom-left corner allows us to eliminate an entire row or column in one step...
  -> This guarantees a linear-time traversal instead of O(m × n)...

Approach:
  > Staircase Search (Greedy Pointer Movement):
     i. Start from the bottom-left cell of the matrix...
     ii. If the current value is negative:
           -> All elements to the right in that row are also negative...
           -> Add (m - currentColumn) to the count...
           -> Move one row up...
     iii. If the current value is zero or positive:
           -> All elements above in that column are non-negative...
           -> Move one column to the right...
     iv. Continue until indices go out of bounds...

Algorithm Steps:
  -> Step 1: Initialize row pointer i = last row index...
  -> Step 2: Initialize column pointer j = 0 (first column)...
  -> Step 3: Initialize result counter res = 0...
  -> Step 4: While i ≥ 0 and j < number of columns:
       * If grid[i][j] < 0:
            res += (m - j)...
            i--...
       * Else:
...
Implementation Note:
  -> Traversal never revisits a cell...
  -> Each step eliminates either one full row or one full column...
  -> Works only because of the sorted matrix property...
...
Example:
  -> grid =
       ...
       [ 1,  1, -1, -2 ]
       [ -1, -1, -2, -3 ]...
  -> Start at bottom-left (-1) → count 4 negatives...
  -> Move up, move right accordingly...
  -> Total negatives counted = 8...

Edge Cases:
  -> Matrix with no negative numbers → return 0...
  -> Matrix with all negative numbers → return m × n...
  -> Single row or single column matrices handled correctly...
  -> Zeros are treated as non-negative...
  -> Large grids processed efficiently...
...
Summary:
  -> Efficient staircase-based traversal...
  -> Exploits matrix sorting to skip unnecessary checks...
  -> Simple pointer logic with guaranteed optimal performance...
  -> Ideal solution for large sorted matrices...
