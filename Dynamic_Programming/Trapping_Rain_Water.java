...
/*
    ...
    Description:
      Following program calculates the total units of rainwater trapped between
        bars of varying heights using an optimized two-pointer approach...

    Problem Statement:
      -> Given an array of non-negative integers representing bar heights...
      -> Each bar has width 1 and is positioned at consecutive indices...
      -> Calculate the total units of water that can be trapped after rainfall...
      -> Return the total trapped water as an integer...

    Key Insight:
      -> Water trapped at any position i = min(leftMax, rightMax) - height[i]...
      -> leftMax = maximum bar height to the left of i (inclusive)...
      -> rightMax = maximum bar height to the right of i (inclusive)...
      -> Water is bounded by the shorter of the two surrounding walls...
      -> If either side has no taller bar, no water can be held at that position...

    Example:
      -> height = [0, 1, 0, 2, 1, 0, 1, 3, 2, 1, 2, 1]:
           Total water trapped = 6 units...
      -> height = [4, 2, 0, 3, 2, 5]:
           Total water trapped = 7 units...
      -> height = [5, 0, 5]:
           Water at index 1 = min(5, 5) - 0 = 5 units...

    Two-Pointer Approach:
      -> Use start pointer from the left end and end pointer from the right end...
      -> Maintain leftMax and rightMax as pointers advance inward...
      -> At each step, process the side with the smaller maximum...
      -> The smaller side's water is safely calculable regardless of the other side...

    Why Process the Smaller Side First?
      -> If leftMax < rightMax:
           Water at start is limited by leftMax, not rightMax...
           Even if rightMax grows further, leftMax remains the bottleneck...
           Safe to compute leftMax - height[start] and advance start...
      -> If rightMax <= leftMax:
           Water at end is limited by rightMax...
           Safe to compute rightMax - height[end] and advance end...
      -> Processing the smaller side guarantees correctness without precomputing arrays...

    Algorithm Steps:
      -> Initialize start = 0, end = n - 1, leftMax = 0, rightMax = 0...
      -> While start < end:
           Update leftMax = max(leftMax, height[start])...
           Update rightMax = max(rightMax, height[end])...
           If leftMax < rightMax:
             Add leftMax - height[start] to totalWater...
             Increment start...
           Else:
             Add rightMax - height[end] to totalWater...
             Decrement end...
      -> Return totalWater...

    Step-by-Step Trace (height = [3, 0, 2]):
      -> start=0, end=2: leftMax=3, rightMax=2, leftMax >= rightMax...
           Water at end: 2 - 2 = 0, end = 1...
      -> start=0, end=1: leftMax=3, rightMax=2, leftMax >= rightMax...
           Water at end: 2 - 0 = 2, end = 0...
      -> start >= end → loop ends...
      -> Total water = 2... (wait, expected is 3 for [3,0,2])
           Correction: rightMax = max(2, height[2]) = 2 at step 1...
           At step 2: height[end=1] = 0, rightMax = max(2, 0) = 2, water = 2 - 0 = 2...
           Hmm: but [3,0,2] → answer is 2 (min(3,2)-0 = 2)...

    Comparison With Precomputation Approach:
      -> Precomputation approach:
           Build leftMax[] and rightMax[] arrays in O(n) time and O(n) space...
           Iterate once more to compute trapped water at each index...
           Total: O(n) time, O(n) space...
      -> Two-pointer approach:
           Single pass, no auxiliary arrays...
           Total: O(n) time, O(1) space...
           Superior in space efficiency while matching time complexity...

    Edge Cases:
      -> Single or two elements → no valley possible → return 0...
      -> Strictly increasing heights → no water trapped → return 0...
      -> Strictly decreasing heights → no water trapped → return 0...
      -> Flat plateau (all equal heights) → no water trapped → return 0...
      -> Deep single valley (5, 0, 5) → all water held in one gap → return 5...

    Why leftMax - height[start] Is Never Negative:
      -> leftMax is updated before the subtraction: leftMax = max(leftMax, height[start])...
      -> After update, leftMax >= height[start] always holds...
      -> So leftMax - height[start] >= 0 guaranteed...
      -> Same argument applies symmetrically for rightMax - height[end]...

    Time and Space Complexity:
      -> Time:  O(n) — each element visited at most once by start or end pointer...
      -> Space: O(1) — only four integer variables used regardless of input size...

    Applications:
      -> Flood water volume estimation in terrain modeling...
      -> Container capacity problems in civil engineering...
      -> Signal valley detection in audio or sensor data processing...
      -> Histogram area and containment problems in competitive programming...
