...
Description:
  Following program finds the minimum path sum from the top-left corner to the bottom-right corner of a grid using Dynamic Programming techniques...

Problem Statement:
  -> Given a 2D grid filled with non-negative integers...
  -> Start at the top-left cell (0,0)...
  -> Reach the bottom-right cell (n-1,m-1)...
  -> You may only move:
       • Right  →
       • Down   ↓
  -> Find the path with the minimum sum of values along the path...

Core Idea:
  -> At each cell, the minimum cost to reach it depends on:
       minimum(top cell, left cell) + current cell value...
  -> Build solution progressively using recursion or DP...
...
Invalid Moves Handling:
  ...
  -> Ensures invalid paths are ignored in minimum computation...

Approach Variants Implemented:
...
Logic:
  -> Recursively compute minimum cost from top and left...
  -> Choose minimum and add current cell value...

Steps:
  i. If at (0,0) → return grid value...
  ii. If out of bounds → return MAX_VALUE...
  iii. Compute:
         up   = recursive(i-1, j)
         left = recursive(i, j-1)
  iv. Return grid[i][j] + min(up,left)...
...
Additional Step:
  -> If dp[i][j] already computed → return stored value...

Benefits:
  -> Eliminated overlapping subproblems...
...
Idea:
  -> Build DP table iteratively from start to destination...

Steps:
  i. Create dp[n][m] table...
  ii. dp[0][0] = grid[0][0]...
  iii. For each cell:
         up   = dp[i-1][j]
         left = dp[i][j-1]
         dp[i][j] = grid[i][j] + min(up,left)...
  iv. Return dp[n-1][m-1]...
...
Idea:
  ...
  -> Use two 1D arrays instead of full matrix...

Steps:
  i. Maintain prev[] and curr[] arrays...
  ii. Compute values row by row...
  iii. curr[j] = grid[i][j] + min(prev[j], curr[j-1])...
  iv. Move curr → prev for next row...
...
Why Use Integer.MAX_VALUE?
  -> Represents unreachable path...
  -> Prevents invalid moves from affecting minimum calculation...

Overflow Safety Check:
  -> If minPrev == Integer.MAX_VALUE:
       avoid addition to prevent overflow...
...
Execution Flow:
  -> Start at (0,0)...
  -> Move right/down choosing minimum cumulative cost...
  -> Reach destination with minimal total sum...

Edge Cases:
  -> Single cell grid → return that value...
  -> One row grid → only move right...
  -> One column grid → only move down...
  -> Large values handled safely...
  -> Prevent overflow using MAX_VALUE guard...

Why Dynamic Programming Works:
  -> Problem exhibits overlapping subproblems...
  -> Optimal substructure property...
  -> Each cell depends on previously solved cells...

Applications:
  -> Grid-based navigation problems...
  -> Robotics path planning...
  -> Game AI path cost computation...
  -> Image seam carving...
