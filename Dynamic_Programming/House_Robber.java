...
Description:
  This program solves the classic **House Robber** problem using **four different Dynamic Programming approaches**...
  The objective is to **maximize the total amount of money robbed** without robbing two adjacent houses...
  Each house contains a certain amount of money, and robbing adjacent houses triggers an alarm...

Problem Statement:
  -> Given an integer array `houses[]` where each element represents money in a house...
  -> You cannot rob two adjacent houses...
  -> Find the **maximum amount of money** you can rob without alerting the police...

Core Idea:
  -> At every house, you have exactly **two choices**:
       • Rob the current house and skip the previous one...
       • Skip the current house and move to the previous one...
  -> This creates an **optimal substructure**, making it a perfect DP problem...
...
Approach 1: Pure Recursion (Brute Force):
  solveRecursive(index, houses)
  -> Tries all possible combinations recursively...
  -> For each index:
       • Rob current house → move to index - 2...
       • Skip current house → move to index - 1...
  -> Returns the maximum of both choices...
  -> Very slow due to repeated subproblems...

Approach 2: Memoization (Top-Down DP):
  solveMemoization(index, houses, memo)
  -> Same recursive logic as Approach 1...
  -> Uses an auxiliary `memo[]` array to store already computed results...
  -> Avoids recomputation of overlapping subproblems...
  -> Significantly improves performance...

Approach 3: Tabulation (Bottom-Up DP):
  solveTabulation(houses)
  -> Builds the solution iteratively from smaller subproblems...
  -> dp[i] stores the maximum money that can be robbed up to house i...
  -> Starts from base cases and fills dp array sequentially...
  -> Eliminates recursion overhead...

Approach 4: Space Optimized DP (Best Approach):
  solveSpaceOptimized(houses)
  -> Observes that only the **last two DP states** are required...
  -> Replaces dp[] array with two variables:
       • twoHousesBack → dp[i - 2]...
       • oneHouseBack  → dp[i - 1]...
  -> Achieves optimal time and space complexity...

Algorithm Steps (Optimized Version):
  -> Initialize two variables for dp[i-2] and dp[i-1]...
  -> Iterate from index 2 to n-1...
  -> At each step:
       • Calculate robCurrent = houses[i] + dp[i-2]...
       • Calculate skipCurrent = dp[i-1]...
       • Store max of both...
  -> Update variables for next iteration...
  -> Return final maximum loot...

Edge Case Handling:
  -> Single house → rob it...
  -> Two houses → rob the one with maximum value...
  -> All equal values → rob alternate houses...
  -> Increasing / decreasing values handled naturally by DP...
...
Space Complexity:
  ...
  -> Memoization:      O(n) memo + O(n) stack...
  -> Tabulation:       O(n) dp array...
  -> Space Optimized:  O(1)...

Key Insights:
  -> Greedy alone does NOT work...
  -> Overlapping subproblems demand Dynamic Programming...
  -> Optimal choice at each house depends on previous results...
  -> Space optimization is possible because only last two states matter...

Final Verdict:
  -> **Space Optimized DP** is the best approach...
  -> Fast, memory-efficient, and production-ready...
  -> A foundational DP problem that appears in many variations...
