...
Description:
  Following program demonstrates the solution to the "Partition Equal Subset Sum" problem using Dynamic Programming by reducing it to a Subset Sum problem...

Problem Statement:
  -> You are given an integer array nums[]...
  -> The task is to determine whether the array can be partitioned into two subsets such that:
...
Approach:
  > Reduction to Subset Sum Problem:
     ...
     ii. If total sum is odd:
           → Partition into equal halves is impossible...
     iii. If total sum is even:
           → Target sum for each subset = totalSum / 2...
     iv. Now check if there exists a subset with sum equal to target...

> Key Insight:
  -> If one subset has sum = target (sum/2)...
  -> Then remaining elements automatically form another subset with sum = target...

> DP State Definition:
  -> dp[i][target] represents:
       whether it is possible to achieve sum = target using elements from index 0 to i...

> Recursive Relation:
  -> For each element nums[idx]:

       notTaken = dp[idx-1][target]...
       taken    = dp[idx-1][target - nums[idx]] if nums[idx] <= target...

       dp[idx][target] = notTaken OR taken...
...
> Algorithm Steps:
  ...
  -> Step 2: If sum % 2 != 0:
       return false...
  -> Step 3: Set target = sum / 2...
  -> Step 4: Use Subset Sum DP to check if target is achievable...
  -> Step 5: Return result...

> Tabulation Approach:

  -> Create boolean DP table dp[n][target+1]...

  -> Initialization:
       dp[i][0] = true for all i (sum 0 always possible)...
  ...
  -> Fill DP table:
       ...
       for each index idx from 1 to n-1...
...
> Space Optimization:
  ...
  -> For each number in nums:
       traverse target from right to left:
            ...
            for t = target down to num:
                 dp[t] = dp[t] OR dp[t - num]...

  -> This ensures previous state is preserved...
...
> Edge Cases:
  ...
  -> Odd total sum → always false...
  -> All elements equal → partition possible if count is even...
  -> Large numbers handled efficiently using DP...

> Key Observations:
  -> This is a classic variation of 0/1 Knapsack problem...
  -> Only need to check for half of total sum...
  -> DP avoids exponential subset generation...

> Applications:
  -> Load balancing problems...
  -> Resource partitioning...
  -> Equal distribution scenarios...
  -> Subset and knapsack variations...
