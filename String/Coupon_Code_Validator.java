...
Description:
  This program validates coupon codes based on multiple rules and sorts the valid ones according to predefined business priorities...  
  Coupons belong to specific business categories such as electronics, grocery, pharmacy, and restaurant, each mapped to a priority level...  
  Only active coupons with valid business lines and properly formatted codes are allowed to pass validation...

Problem Statement:
  -> You are given three parallel arrays:  
       * code[]: list of coupon code strings...  
       * businessLine[]: corresponding business category for each code...  
       * isActive[]: boolean array indicating if a coupon is active...  
  -> The task is to validate each coupon using the following rules:  
       1. Business line must be one of: {"electronics", "grocery", "pharmacy", "restaurant"}...  
       2. Coupon must be active (isActive[i] = true)...  
       3. Coupon code must be valid according to character rules...  
  -> After validation, return all valid coupons sorted first by business priority, then lexicographically...

Valid Code Rules:
  -> Code cannot be an empty string...  
  -> Code may contain only:  
       * Letters (A–Z, a–z)...  
       * Digits (0–9)...  
       * Underscore (_)…  
  -> Any special character like @, #, $, space, etc. invalidates the code...
...
Approach:
  > Step 1: Maintain a HashMap<String,Integer> to map business lines to their priority values...  
  > Step 2: Iterate through all coupons and apply the following filters:  
       * Skip coupon if inactive...  
       * Skip coupon if businessLine is not in the priority map...  
       * Skip coupon if isValidCode() returns false...  
  > Step 3: For each valid coupon, store a Pair(priority, code)...  
  > Step 4: Sort all valid pairs using comparator rules:  
       i. Compare priority first (ascending)...  
       ii. If same priority, compare coupon codes alphabetically...  
  > Step 5: Extract sorted codes into ArrayList<String> and return...

Algorithm Details:
  -> isValidCode():
       * Loop through characters of the string...  
       * Check if each char is letter, digit, or underscore...  
       * If any invalid character found → return false...  
       * Otherwise → return true...  
  -> validateCoupons():
       * Build list of valid Pair objects...  
       * Sort the list according to priority rules...  
       * Extract only codes and return...

Example:
  Input:
    code[] = {"REST100", "PHARM50", "ELEC200", "GRO30"}...  
    businessLine[] = {"restaurant", "pharmacy", "electronics", "grocery"}...  
    isActive[] = {true, true, true, true}...  
  Output:
    ["ELEC200", "GRO30", "PHARM50", "REST100"]...  
  Explanation:
    Sorted by business priority first, then alphabetically when priorities match...

Edge Cases:
  -> Empty code string → invalid...  
  -> Invalid businessLine → ignored...  
  -> Inactive coupon → ignored...  
  -> Codes with spaces or special characters → invalid...  
  -> All coupons invalid → return empty list...  
  -> All business lines invalid → return empty list...

Time & Space Complexity:
  -> Validation: O(n * m) where m = average code length...  
  -> Sorting: O(n log n)...  
  -> Space: O(n) for storing valid pairs...

Summary:
  -> Ensures coupon formatting correctness...  
  -> Filters active coupons only...  
  -> Applies business-based priority ordering...  
  -> Produces deterministic sorted output based on business rules...  
